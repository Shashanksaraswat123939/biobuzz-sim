...
export const CONTROLS: ControlGroup[] = [
  {
    title: 'Driving',
    blurb: 'Mecanum, and ROBOT-CENTRIC: forward on the left stick is the robot’s NOSE, which is where the intake is. Almost everything you do with the chassis is aim the intake at a ball, and a field-relative stick makes that a mental rotation on every approach. The turret is why this costs nothing — the chassis never has to face the goal, so the nose is free to mean “where I am collecting from” all match. Hold Ctrl (R3) for field-centric if you prefer it.',
    items: [
      { key: 'W S', pad: 'left stick ↕', what: 'Forward and back, along the nose', why: 'On a keyboard the stick ramps to full over about a fifth of a second rather than stepping, because a key has no middle and a 1.2 m/s drivetrain asked for in one step is undriveable. The wheels model slip, so flooring it on a low tile friction spins them instead of accelerating.' },
      ...
      { key: 'Q E', pad: 'X / B', what: 'Turn left and right', why: 'Rotates in place. Mixed with the left stick it arcs. The buttons ramp exactly as the keyboard sticks do, so a press does not slam the chassis to full rotation in one frame.' },
      { key: 'R F', pad: 'Y / A', what: 'Drive speed gear, up and down', why: 'Five gears. The bar under the status strip reads the top speed the gear allows, in m/s, worked out from the drive motor’s free speed and the wheel radius — not written down anywhere. A gear is a setting, so it survives letting go of the stick, which is what “crawl for the last six inches, then go” needs.' },
      { key: 'Shift', pad: 'LB', what: 'Momentary crawl', why: 'Scales every drive command to 35% for as long as you hold it, on top of whatever gear you are in. For a dab of precision without changing gear.' },
      { key: 'Ctrl', pad: 'R3', what: 'Field-centric while held', why: 'Forward becomes away-from-the-driver-station whatever way the robot points, measured from the last re-zero.' },
      { key: 'Backspace', pad: 'L3', what: 'Re-zero the field frame', why: 'Makes “away” mean whichever way the robot faces now. Square it up to the field and tap it; a real IMU drifts and every driver wants this button.' },
    ],
  },
  {
    title: 'Shooting',
    ...
    items: [
      { key: 'Space', pad: 'R1', what: 'Fire, while held', why: 'Your own trigger. Balls go as fast as the shooter can honestly cycle them: the wheel has to be inside its tolerance band, the turret on target, and the CELL mouth has to be open towards you. Let go and it stops.' },
      { key: 'T', pad: 'M1', what: 'Auto-aim on/off', why: 'On: the turret and hood solve for your up CELL every loop, including a lead for the robot’s own velocity, so you can shoot while driving across the shot. Off: the turret holds wherever you last pointed it. M1 is a paddle; on a pad without paddles it is L1.' },
      { key: 'G', pad: 'M2', what: 'Auto-fire on/off', why: 'A latch, not a trigger: leave it on and a ball goes every time the shot clears the gate, paced by the transfer cycle time rather than your thumb. M2 falls back to L2 on a pad without paddles. Both are also buttons on the action bar.' },
      ...
      { key: 'Z', pad: 'L2', what: 'Reverse the intake', why: 'Spits out whatever is at the mouth. The same traction model that pulls a ball in pushes it back out; there is no separate eject.' },
    ],
  },
  {
    title: 'View',
